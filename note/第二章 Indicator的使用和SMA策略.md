...
![image-20221128214101286](assets/image-20221128214101286.png)

lines是一个元组，所以在'sma'后面要加一个逗号。

在指标的next函数中，需要做的是将最开始lines中定义的“指标中有哪些lines”的值都算出来。

`self.data.get(size=xxx)`是从当前时间开始向前取多少size个数据。

<img src="assets/image-20221128214923052.png" alt="image-20221128214923052" style="zoom:50%;" />
...
![image-20221129155802486](assets/image-20221129155802486.png)

我们并不需要在图中显示下面crossover和crossdown的图像，可以设置指标类的`__init__`函数：
...
![image-20221129161222714](assets/image-20221129161222714.png)

如果连图中的箱体线up和down都不要，那么可以在init函数中添加一句：

```python
self.up_down.plotinfo.plot = False
```

如果将指标线up和down跟主图分离，那就注释掉：
...
![image-20221129162351429](assets/image-20221129162351429.png)
